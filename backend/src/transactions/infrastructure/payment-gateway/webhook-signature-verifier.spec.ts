...
function computeChecksum(concatenatedProperties: string, timestamp: number, secret: string): string {
...
describe('WebhookSignatureVerifier', () => {
  it('returns true when the checksum matches, resolving properties in order from data', () => {
    ...
    const data = { transaction: { id: 'tx_1', status: 'APPROVED', amount_in_cents: 5000 } };
    const timestamp = 1530291411;
    const properties = ['transaction.id', 'transaction.status', 'transaction.amount_in_cents'];
    const checksum = computeChecksum('tx_1APPROVED5000', timestamp, config.eventsKey);
  ...
  it('compares the checksum case-insensitively', () => {
    ...
    const result = verifier.verify({
      ...
      data: { transaction: { id: 'tx_1' } },
      timestamp,
      signature: { properties: ['transaction.id'], checksum: checksum.toLowerCase() },
    });
